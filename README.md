...
return [
    //
    'applicationId' => 'Your Hossapp Application Id',
    'logBody' => true,
];
```

Your Hossapp Application Id can be found in the [_Hossapp Portal_](https://www.hossapp.com/).
After signing up for a Hossapp account, your Hossapp Application Id will be displayed during the onboarding steps.

You can always find your Hossapp Application Id at any time by logging
into the [_Hossapp Portal_](https://www.hossapp.com/), click on the top right menu,
...
You can define Hossapp configuration options in the `config/hossapp.php` file.
...
#### __`logBody`__
Type: `Boolean`
Optional, Default true, Set to false to remove logging request and response body to Hossapp.
...
return [
    ...
    'configClass' => 'MyApp\\MyConfigs\\CustomHossappConfig',
    ...
];
```
...
Because configuration hooks and functions cannot be placed in the `config/hossapp.php` file, these reside in a PHP class that you create.
...
Optional, a function that takes a $request and $response and return a string for userId. Hossapp automatically obtains end userId via $request->user()['id'], In case you use a non standard way of injecting user into $request or want to override userId, you can do so with identifyUserId.
...
Type: `($request, $response) => String`
Optional, a function that takes a $request and $response and return a string for sessionId. Hossapp automatically sessionizes by processing at your data, but you can override this via identifySessionId if you're not happy with the results.
...
this API call should be not be sent to Hossapp.
...
Create or update a user profile in Hossapp.
The metadata field can be any customer demographic or other info you want to store.
Only the `user_id` field is required.

```php
use Hossapp\Middleware\HossappLaravel;

// Only userId is required.
// Campaign object is optional, but useful if you want to track ROI of acquisition channels
// See https://www.hossapp.com/docs/api#users for campaign schema
...
Create or update a company profile in Hossapp.
...
// Campaign object is optional, but useful if you want to track ROI of acquisition channels
// See https://www.hossapp.com/docs/api#update-a-company for campaign schema
// metadata can be any custom object
$company = array(
    "company_id" => "67890",
    "company_domain" => "acmeinc.com", // If domain is set, Hossapp will enrich your profiles with publicly available info
    "campaign" => array(
        "utm_source" => "google",
        "utm_medium" => "cpc",
        "utm_campaign" => "adwords",
...
## Credits for Hossapp Laravel SDK
...
- The forked (i.e. non-blocking way) of sending data is using exec() with a cURL command. The Php exec() command can be successful but the cURL itself may have 401 errors.  So after integration, if you don't see events and data show up in your Hossapp Dash. Please turn on debug option, then the cURL command itself will logged. You can execute that cURL command and see what the issues are. The most common thing to check is if the Application ID is set correctly.

In case you've exec() as a disabled function, you could set configuration option `disableForking` to `true` to send data to Hossapp using curl PHP extension.

## Troubleshooting

### exec() must exist/exec() must be enabled
By default, Hossapp forks a process to log API calls in an asynchronous method, which requires your PHP environment to have exec() enabled. For highest performance, the recommended fix is to ensure exec() is enabled for your hosting environment:
...
If you cannot enable exec (such as for shared hosting environments), you can disable forking by adding the following to your `hossapp.php`. 
...
This error happens when you disabled forking and you do not have the cURL PHP extension enabled. The recommended fix is to enable forking by setting `disableForking` to false in your `hossapp.php`. Otherwise, ensure you have enabled the PHP CURL extension. [More info](https://www.php.net/manual/en/curl.installation.php).

### No events show up in Hossapp
Because Hossapp forks a process, you may not see all errors from the child process. A common case for event not showing up is due to an incorrect application id.
To see debug logs, you can add the following to your `hossapp.php`:
...
## Test Laravel App with Hossapp Integrated

[Hossapp Laravel Tests](https://github.com/Hossapp/hossapp-laravel-tests)

## An Example Laravel App with Hossapp Integrated

[Hossapp Laravel Example](https://github.com/Hossapp/hossapp-laravel-example)
...
To view more documentation on integration options, please visit __[the Integration Options Documentation](https://www.hossapp.com/docs/getting-started/integration-options/).__

[ico-built-for]: https://img.shields.io/badge/built%20for-laravel-blue.svg
[ico-version]: https://img.shields.io/packagist/v/hossapp/hossapp-laravel.svg
[ico-downloads]: https://img.shields.io/packagist/dt/hossapp/hossapp-laravel.svg
[ico-license]: https://img.shields.io/badge/License-Apache%202.0-green.svg
[ico-source]: https://img.shields.io/github/last-commit/hossapp/hossapp-laravel.svg?style=social

[link-built-for]: http://laravel.com
[link-package]: https://packagist.org/packages/hossapp/hossapp-laravel
[link-downloads]: https://packagist.org/packages/hossapp/hossapp-laravel
[link-license]: https://raw.githubusercontent.com/Hossapp/hossapp-laravel/master/LICENSE
[link-source]: https://github.com/hossapp/hossapp-laravel
...
so the SDK configuration has changed in v2.x.x. To migrate, you will need to move any functions from your `config/hossapp.php` into a separate class such as CustomHossappConfig.
...
- In your `hossapp.php` in the config folder:
