...
class HossappApi extends BaseClass {
    ...
    /**
     * Instantiates a new HossappApi instance.
     * @param $applicationId
     * @param array $options
     */
    ...
    /**
     * Returns a singleton instance of HossappApi
     * @param $applicationId
     * @param array $options
     * @return HossappApi
     */
    public static function getInstance($applicationId, $options = array()) {
        if(!isset(self::$_instance)) {
            self::$_instance = new HossappApi($applicationId, $options);
        }
        return self::$_instance;
    }
    /**
     * Add an array representing a message to be sent to Hossapp to the in-memory queue.
     * @param array $message
     */
    ...
    /**
     * Add an array representing a list of messages to be sent to Hossapp to a queue.
    ...
    public function updateUser($userData) {
      if (is_null($userData)) {
        throw new Exception('Hossapp UpdateUser with a null userData object');
      }

      if (!isset($userData['user_id'])) {
        throw new Exception('Hossapp updateUser requires user_id field to be set');
      }
    ...
    public function updateCompany($companyData) {
      if (is_null($companyData)) {
        throw new Exception('Hossapp UpdateCompany with a null companyData object');
      }

      if (!isset($companyData['company_id'])) {
        throw new Exception('Hossapp updateCompany requires company_id field to be set');
      }
