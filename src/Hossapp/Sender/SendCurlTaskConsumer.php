...
class SendCurlTaskConsumer extends SendTaskConsumer {
    ...
    function __construct($applicationId, $options) {
        ...
        if ($this->_fork == true) {
            ...
            if (!$exists) {
                throw new Exception('exec() must exist to log API calls via a forked process. Either enable exec() in your PHP environment (recommended) or disable forking by setting \'disableForking\' => true to your hossapp.php.');
            }
            $disabled = explode(', ', ini_get('disable_functions'));
            $enabled = !in_array('exec', $disabled);
            if (!$enabled) {
                throw new Exception('exec() must be enabled to log API calls via a forked process. Either enable exec() in your PHP environment (recommended) or disable forking by setting \'disableForking\' => true to your hossapp.php.');
            }
        } else {
            if (!function_exists('curl_init')) {
                throw new Exception('The PHP cURL extension is required when disableForking is true in your hossapp.php. Either allow forking or ensure the cURL extension exists.');
            }
        }
    }
    /**
    ...
    protected function _execute($url, $data) {
        ...
        $events = (object) ['events' => json_decode($data)];
        $batch = json_encode($events);
        ...
        $headers = [
            ...
            'Authorization: Bearer ' . $this->_appId,
            'User-Agent: hossapp-laravel/2.0.6'
        ];
    ...
    protected function _execute_forked($url, $data) {
		...
		if(strcasecmp(substr(PHP_OS, 0, 3), 'WIN') == 0) {
			$exec = ' echo ' . $batch . ' '.' | '.'curl -v -X POST -H "Content-Type: application/json" -H "User-Agent: hossapp-laravel/2.0.6" -H "Authorization: Bearer ' . $applicationId .'" -d @- "' . $url . '"';
